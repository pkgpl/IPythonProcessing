...
@jit
def t_side1(time,s,h,ix,iz,ta):
    time[ix,iz]=ta+h*s[ix,iz]

@jit
def t_bottom1(time,s,h,ix,iz):
    ta=time[ix,iz-1]
    t_side1(time,s,h,ix,iz,ta)
    
@jit
def t_top1(time,s,h,ix,iz):
    ta=time[ix,iz+1]
    t_side1(time,s,h,ix,iz,ta)

@jit
def t_left1(time,s,h,ix,iz):
    ta=time[ix+1,iz]
    t_side1(time,s,h,ix,iz,ta)
    
@jit
def t_right1(time,s,h,ix,iz):
    ta=time[ix-1,iz]
    t_side1(time,s,h,ix,iz,ta)
...
@jit
def t_corner(time,s,h,ix,iz,ta,tb,tc):
    tmp=2.*(h*s[ix,iz])**2-(tb-tc)**2
    if tmp<0.:
...
@jit
def t_bottom_right(time,s,h,ix,iz):
    ta=time[ix-1,iz-1]
    tb=time[ix-1,iz]
    tc=time[ix,iz-1]
...
@jit
def t_bottom_left(time,s,h,ix,iz):
    ta=time[ix+1,iz-1]
    tb=time[ix+1,iz]
    tc=time[ix,iz-1]
    t_corner(time,s,h,ix,iz,ta,tb,tc)
...
@jit
def t_top_right(time,s,h,ix,iz):
    ta=time[ix-1,iz+1]
    tb=time[ix-1,iz]
    tc=time[ix,iz+1]
    t_corner(time,s,h,ix,iz,ta,tb,tc)
...
@jit
def traveltime(vel,h,srcx,srcz):
    nx,nz=vel.shape
    s=1./vel

    tinit=2**10
    time=np.ones((nx,nz))*tinit

    boundary_top=srcz
    boundary_bottom=srcz
    boundary_left=srcx
    boundary_right=srcx
